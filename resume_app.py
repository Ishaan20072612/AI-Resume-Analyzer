...
<style>
    ...
    .tag-yellow { background: #1f1a07; color: #fbbf24; border: 1px solid #713f12; }
    .hire-badge {
        display: inline-block;
        padding: 8px 20px;
        border-radius: 6px;
...
st.markdown('<div class="hero-sub">AI-powered · Instant feedback · ATS optimized</div>', unsafe_allow_html=True)
...
if analyze_btn:
    ...
    with st.spinner("Analyzing your resume..."):
        try:
            ...
            if not resume_text:
                st.error("Could not extract text from PDF. Please ensure the PDF is not scanned/image-only.")
                st.stop()
    ...
    with score_col:
        ...
        <div style="text-align:center">
            <div style="font-family:'DM Mono',monospace;font-size:0.7rem;color:#555;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px">Hire Recommendation</div>
            <span class="hire-badge {hire_class}">{hire}</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with verdict_col:
        ...
        if strengths:
            st.markdown('<div class="section-card"><div class="section-title">Strengths</div>' +
                "".join([f'<span class="tag tag-green">{s}</span>' for s in strengths]) +
                '</div>', unsafe_allow_html=True)

    # Missing skills + Improvements + ATS
